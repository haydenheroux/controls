...
struct PositionVelocityState : public VectorBase<PositionVelocityState, 2> {
  ...
  PositionVelocityState() : PositionVelocityState(au::meters(0), (au::meters / au::second)(0)) {}

  PositionVelocityState(const StateVector<Dimension>& state)
      : PositionVelocityState(au::meters(state[0]), (au::meters / au::second)(state[1])) {}
  ...
  bool At(const PositionVelocityState& other) const {
    ...
    bool velocity_in_tolerance =
        au::abs(Velocity() - other.Velocity()) < (au::centi(au::meters) / au::second)(1);
 ...
 */
struct PositionAccelerationState : public VectorBase<PositionAccelerationState, 2> {
  StateVector<Dimension> vector;

  PositionAccelerationState(LinearVelocity velocity, LinearAcceleration acceleration) {
    SetVelocity(velocity);
    SetAcceleration(acceleration);
  }

  PositionAccelerationState(LinearVelocity velocity)
      : PositionAccelerationState(velocity, (au::meters / au::squared(au::second))(0)) {}
  ...
  PositionAccelerationState(const StateVector<Dimension>& state)
      : PositionAccelerationState((au::meters / au::second)(state[0]),
  ...
  LinearVelocity Velocity() const { return (au::meters / au::second)(vector[0]); }
  LinearAcceleration Acceleration() const { return (au::meters / au::squared(au::second))(vector[1]); }

  void SetVelocity(LinearVelocity velocity) { vector[0] = velocity.in(au::meters / au::second); }
  void SetAcceleration(LinearAcceleration acceleration) { vector[1] = acceleration.in(au::meters / au::squared(au::second)); }
};
 ...
 * Here we provide the concrete mapping so `TimeDerivative<PositionVelocityState>`
 * resolves to `PositionAccelerationState`.
 */
template <>
...
static_assert(std::is_same_v<TimeDerivative<PositionVelocityState>, PositionAccelerationState>,
              "TimeDerivative mapping for PositionVelocityState must be PositionAccelerationState");

static_assert(PositionAccelerationState::Dimension == PositionVelocityState::Dimension,
              "PositionAccelerationState must have the same Dimension as PositionVelocityState");

static_assert(HasDimension<PositionVelocityState>, "PositionVelocityState must satisfy HasDimension");
static_assert(HasDimension<PositionAccelerationState>, "PositionAccelerationState must satisfy HasDimension");
