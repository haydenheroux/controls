...
int main() {
  ...
  for (double gear_ratio = min_gear_ratio; gear_ratio <= max_gear_ratio;
       gear_ratio += 0.01) {
    Elevator elevator{units::gear_ratio(gear_ratio),
                      DRUM_RADIUS,
                      MASS,
                      MAX_CURRENT,
                      TOTAL_TRAVEL,
  ...
  Elevator elevator{units::gear_ratio(best_gear_ratio),
                    DRUM_RADIUS,
                    MASS,
                    MAX_CURRENT,
                    TOTAL_TRAVEL,
  ...
  quantities::LinearVelocity maximum_velocity =
      MaximumVelocity<Elevator, units::DisplacementUnit>(elevator);

  std::cout << "Optimal Gear Ratio: " << best_gear_ratio
            << "\nMaximum Velocity: " << maximum_velocity
            << "\nMaximum Acceleration: "
            << MaximumAcceleration<Elevator, units::DisplacementUnit>(elevator)
            << "\nLimit Voltage: "
            << LimitVoltage<Elevator, units::LinearVelocityUnit>(
                   elevator, maximum_velocity, au::volts(12))
            << "\nMaximum Current: "
            << reefscape::Current<Elevator, units::LinearVelocityUnit>(
                   elevator, (au::meters / au::second)(0), au::volts(12))
